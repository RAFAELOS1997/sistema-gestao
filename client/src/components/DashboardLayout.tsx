...
import { LayoutDashboard, LogOut, Package, PanelLeft, ShoppingCart, ShoppingBag, BarChart3, Settings, Truck, Users as UsersIcon, Receipt, ClipboardCheck, Sparkles, Handshake, Wallet, ClipboardList, Store, UserCircle, Gem } from "lucide-react";
...
const menuItems = [
  ...
  { icon: Gem, label: "Sabedoria dos Cristais", path: "/sabedoria-dos-cristais" },
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <SidebarMenuItem key={item.path}>
                    ...
                    >
                      <item.icon className={`h-4 w-4 shrink-0 ${isActive ? "text-accent" : "text-muted-foreground"}`} />
                      <span className={isActive ? "text-accent font-medium" : ""}>{item.label}</span>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                );
              })}
